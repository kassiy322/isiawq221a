...
class CSVWriter(FileWriter):
    ...
    def _data_mapping(self) -> dict[str, Any]:
        data_mapping = {
            ...
            # Новые поля
            'building_id': 'ID здания', 'building_name': 'Название здания',
            'poi_category': 'Категория POI', 'updated_at': 'Дата обновления',
            'has_photos': 'Есть фото', 'photo_count': 'Количество фото',
            'is_promoted': 'Промотируется', 'has_discount': 'Есть скидки',
            'attributes': 'Атрибуты', 'nearest_station': 'Остановка рядом',
            'station_distance': 'Расстояние до остановки'
        }
    ...
    def _extract_raw(self, catalog_doc: Any) -> dict[str, Any]:
        ...
        # Фотографии
        if catalog_item.flags:
            data['has_photos'] = catalog_item.flags.photos
            
        if catalog_item.external_content:
            for content in catalog_item.external_content:
                if content.type == 'photo_album':
                    data['photo_count'] = content.count
                    break
                    
        # Флаги объекта
        data['is_promoted'] = catalog_item.is_promoted
        data['has_discount'] = catalog_item.has_discount
        
        # Атрибуты
        if catalog_item.attribute_groups:
            attributes = []
            for group in catalog_item.attribute_groups:
                for attr in group.attributes:
                    attributes.append(attr.name)
            if attributes:
                data['attributes'] = self._options.csv.join_char.join(attributes)
                
        # Остановки рядом
        if catalog_item.links and catalog_item.links.nearest_stations:
            nearest = catalog_item.links.nearest_stations[0]  # Ближайшая
            data['nearest_station'] = nearest.name
            data['station_distance'] = nearest.distance
