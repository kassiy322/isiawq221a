...
def default_max_records() -> int:
    ...
    Updated to support pagination - increased default to allow
    multiple pages to be processed automatically.
    """
    max_records = floor_to_hundreds((550 * default_memory_limit() / 1024 - 400))
    # Ensure minimum of 50 records to enable pagination across multiple pages
    return max(max_records, 50) if max_records > 0 else 50
